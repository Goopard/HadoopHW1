...
class JobTest(unittest.TestCase):
    def setUp(self):
        ...
        with open(os.path.join(path, 'test_inputs', 'test_one_longest_word_from_some_text_input.txt'), 'rb') as file:
            self.first_test_stdin = BytesIO(file.read())
        with open(os.path.join(path, 'test_inputs', 'test_two_longest_words_from_some_text_input.txt'), 'rb') as file:
